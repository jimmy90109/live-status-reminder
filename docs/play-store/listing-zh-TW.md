# Google Play 商店資訊（繁體中文）

## App 名稱

即時狀態提醒

## 簡短說明

將乘車、外送與種花通知轉成清楚、持續顯示的 Android 即時狀態提醒。

## 完整說明

不再反覆打開 App 確認狀態。

即時狀態提醒會在您的裝置上辨識 iPASS MONEY、foodpanda、Uber、Uber Eats 與 Pikmin Bloom 通知，將重要進度整理成持續顯示的 Android Live Update：

- iPASS MONEY：進站後提醒您在下車前準備開啟乘車碼，出站後自動結束提醒。
- foodpanda：顯示外送中與即將抵達狀態，送達或取消後自動結束提醒。
- Uber：顯示上車時間、上車點、車輛 PIN 與下車點，行程完成後自動結束提醒。
- Uber Eats：顯示從接單、備餐、取餐、配送到即將抵達的進度；通知有提供交付 PIN 時，也能在解鎖後快速查看。
- Pikmin Bloom：背景種花時提醒您記得結束種花，避免花瓣在原地耗盡。
- 點一下提醒即可開啟對應 App。

隱私優先：

- 所有通知辨識都只在您的裝置上進行。
- 不上傳、出售或分享通知內容與 PIN。
- 不永久儲存通知內容與 PIN。
- 您可以隨時在 Android 系統設定關閉通知存取。

系統需求：Android 16 或更新版本。Live Update 顯示方式依裝置支援情況而定。

本 App 與 iPASS MONEY、foodpanda、Uber、Uber Eats、Pikmin Bloom 及其所屬公司沒有隸屬、授權或合作關係；相關商標均屬各自權利人所有。

## 分類建議

工具

## 標籤建議

通知、效率、交通、外送

## 發布版本名稱

1.0.0-beta3

## 發布說明

1.0.0-beta3 測試版本：

- 支援 iPASS MONEY 乘車進出站提醒。
- 支援 foodpanda 外送進度提醒。
- 支援 Uber 乘車進度與 PIN 顯示。
- 支援 Uber Eats 訂單進度與交付 PIN 顯示。
- 支援 Android 16 Live Update。
- 通知內容僅在裝置上處理，不上傳或分享。
